...
Type inference flows from the outside in via `let` annotations and function return types. It does not flow from a function's parameter types back into its arguments (see [#115](https://github.com/metel-lang/metel/issues/115)), and it does not flow from a match expression's expected type into arm bodies (see [#114](https://github.com/metel-lang/metel/issues/114)). In those positions there is no binding to annotate, so ascription is the only inline option.
