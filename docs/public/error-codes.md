...
```
[P0001] parse error in main.yolo at 12..18 (`let x = ;`): expected expression
```
...
```
[P0002] parse error in main.yolo at 4..24: integer literal '99999999999999999999' is out of range for i64
```
...
```
[P0003] parse error in main.yolo at 4..12: invalid float literal '1e9999'
```
...
```
[T0001] type error in main.yolo at 10..20: expected Int, got Bool
```
...
```
[T0002] type error in main.yolo at 5..10: cannot infer type of `x`; add a type annotation
```
...
```
[T0003] type error in main.yolo at 8..12: undefined name `foo`
```
...
```
[T0004] type error in main.yolo at 5..20: expected 2 arguments, got 3
```
...
```
[T0005] type error in main.yolo at 6..13: operator `+` cannot be applied to Bool and Int
```
...
```
[T0006] type error in main.yolo at 3..12: `x` is immutable; use `mut x` to allow reassignment
```
...
```
[T0007] type error in main.yolo at 5..15: cannot cast Bool to Int
```
...
```
[T0008] type error in main.yolo at 2..30: match on Colour is non-exhaustive; missing variant `Blue`
```
...
```
[R0001] runtime error in main.yolo at 0..0: no main() function defined
```
...
```
[R0002] runtime error in main.yolo at 0..0: main() is generic — not supported in v0.1
```
...
```
[R0003] runtime error in main.yolo at 10..15: undefined variable `x`
```
...
```
[R0004] runtime error in main.yolo at 5..10: index 5 out of bounds (len 3)
```
...
```
[R0005] runtime error in main.yolo at 5..10: tuple index 3 out of bounds
```
...
```
[R0006] runtime error in main.yolo at 2..30: match: no arm matched scrutinee
```
...
```
[R0007] runtime error in main.yolo at 8..13: division by zero
```
...
```
[R0008] runtime error in main.yolo at 5..12: no field `colour` on value
```
...
```
[R0009] runtime error in main.yolo at 5..20: no method `draw` on `Circle`
```
...
```
[R0010] runtime error in main.yolo at 3..8: call: expected a closure or builtin
```
...
```
[R0011] runtime error in main.yolo at 1..20: for-in: expected Array or Range
```
...
```
[R0012] runtime error in main.yolo at 5..10: ?: expected a Result value
```
