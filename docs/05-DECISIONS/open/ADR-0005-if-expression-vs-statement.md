...
- `Stmt::If` and `TypedStmt::If` are removed from the AST
- Grammar: `if_stmt` rule deleted; `if_expr` gains optional else and supports `else if`
  chains
- Parser: produces `Expr::If` for all `if` constructs; statement position wraps it in
  `Stmt::Expr`
- Typechecker: `infer_expr`/`construct_expr` for `Expr::If` must handle the optional
  `else_branch`; no-else `if` produces `Unit`
- Evaluator: handles `Expr::If` with optional else (else branch absent → unit value)
- The Stage 2 test programs using `if (cond)` directly as a block tail now type-check

**If Option C is chosen:**
- Add a spec note in §4 (Statements) and §5 (Expressions) documenting the
  `if`-in-block parsing rule and the `let`-binding workaround
