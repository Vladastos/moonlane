//! Integration tests: each .mtl test program must run through the parser without errors. These tests are meant to cover the full range of language features, and are not expected to be minimal. They are primarily intended to catch regressions in the parser as new features are added.
...
#[test]
fn test_01_literals_and_variables() { run("01_literals_and_variables.mtl"); }

#[test]
fn test_02_control_flow() { run("02_control_flow.mtl"); }

#[test]
fn test_03_functions_and_closures() { run("03_functions_and_closures.mtl"); }

#[test]
fn test_04_structs_and_impl() { run("04_structs_and_impl.mtl"); }

#[test]
fn test_05_enums_and_match() { run("05_enums_and_match.mtl"); }

#[test]
fn test_06_traits() { run("06_traits.mtl"); }

#[test]
fn test_07_arrays_and_tuples() { run("07_arrays_and_tuples.mtl"); }

#[test]
fn test_08_error_handling() { run("08_error_handling.mtl"); }

#[test]
fn test_09_casting_and_generics() { run("09_casting_and_generics.mtl"); }

#[test]
fn test_10_comprehensive() { run("10_comprehensive.mtl"); }

#[test]
fn test_11_block_expr_stmts() { run("11_block_expr_stmts.mtl"); }
...
fn parse_error_message(filename: &str) -> String {
    ...
    let source = std::fs::read_to_string(&path)
        .unwrap_or_else(|e| panic!("could not read {}: {}", path, e));
    match metel::parser::parse(&source, filename) {
        Err(e) => format!("{}", e),
        Ok(_) => panic!("expected a parse error from {filename} but parsing succeeded"),
    }
}

// neg_01_syntax_error.mtl: `@@@` on line 3, col 1 → P0001
...
// neg_02_int_overflow.mtl: oversized integer at line 1, col 14 → P0002
...
// neg_03_float_invalid.mtl: exponent float (unsupported syntax) at line 1, col 17
...
// neg_04_error_at_line_10.mtl: `@@@` on line 10 — verifies line counting past line 9
